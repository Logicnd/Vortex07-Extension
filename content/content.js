...
const VORTEX07_SCARY_USER_IDS = new Set([22795]); // erik2 — creepypasta myth

function isScaryVortexUser(userId) {
  const id = safeNumber(userId);
  return id !== null && VORTEX07_SCARY_USER_IDS.has(id);
}

let scaryMythLoadPromise = null;

function shouldLoadScaryMythPayload() {
  if (location.hash === "#backdoor-22795") return true;
  if (isScaryVortexUser(getProfileUserIdFromPage?.())) return true;
  if (isScaryVortexUser(getLoggedInUserIdFromNav?.())) return true;
  return false;
}

function ensureScaryMythStylesheet() {
  const id = "vortex07-scary-myth-css";
  ...
  return new Promise((resolve) => {
    ...
    link.id = id;
    link.rel = "stylesheet";
    link.href = extensionAssetUrl("styles/features/scary-myth.css");
    link.onload = () => resolve();
    link.onerror = () => {
      logError("Failed to load scary-myth.css");
      resolve();
    };
...
function ensureScaryMythScript() {
  if (typeof spreadScaryMythAcrossSite === "function") return Promise.resolve();

  return new Promise((resolve, reject) => {
    const script = document.createElement("script");
    script.src = extensionAssetUrl("content/features/scary-myth.js");
    script.onload = () => resolve();
    script.onerror = () => reject(new Error("scary-myth.js failed to load"));
    (document.head || document.documentElement).appendChild(script);
  });
}

function ensureScaryMythLoaded() {
  if (!shouldLoadScaryMythPayload()) return Promise.resolve(false);
  if (!scaryMythLoadPromise) {
    scaryMythLoadPromise = Promise.all([
      ensureScaryMythStylesheet(),
      ensureScaryMythScript(),
    ]).then(() => true);
  }
  return scaryMythLoadPromise;
}

function runScaryMythEnhancements() {
  void ensureScaryMythLoaded().then((loaded) => {
    if (!loaded) return;
    if (typeof ensureScaryMythRouting === "function") ensureScaryMythRouting();
    if (typeof spreadScaryMythAcrossSite === "function") spreadScaryMythAcrossSite();
  });
}
...
function ensureForumStylesheet() {
  ...
  return new Promise((resolve) => {
    ...
    link.id = id;
    link.rel = "stylesheet";
    link.href = extensionAssetUrl("styles/features/forum-rbx.css");
    link.onload = () => resolve();
    link.onerror = () => {
      logError("Failed to load forum-rbx.css");
      resolve();
    };
    document.documentElement.appendChild(link);
  });
...
function syncThemeStylesheet(enabled) {
  const STYLE_SHEETS = [
    { id: "vortex07-theme-css", path: "styles/styles.css", label: "styles.css" },
    { id: "vortex07-retro-css", path: "styles/retro-2007.css", label: "retro-2007.css" },
  ];
